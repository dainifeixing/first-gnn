...
def score_gnn_directory(
    ...
    if selected_top_rows:
        recommendations.append("Export review candidates from the current score file for manual confirmation.")
        if summary["max_top_score"] >= 0.9:
            recommendations.append("Start review from the highest-score workbook and preserve confirmed outcomes as next-round labels.")
    else:
    ...
    if csv_path:
        ...
        with path.open("w", encoding="utf-8", newline="") as handle:
            ...
            for item in candidates:
                writer.writerow({key: item.get(key, "") for key in review_fields})

    if xlsx_path:
        write_xlsx_table(
            xlsx_path,
            headers=review_fields,
            rows=[{key: item.get(key, "") for key in review_fields} for item in candidates],
